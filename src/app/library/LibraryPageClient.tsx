...
export interface LibraryPageProps {
  ...
  sadhanaLibrary: Sadhana[];
  sadhanaCount: number;
  siddhiCount: number;
  coursePhaseCount: number;
  categoryMap: Record<string, TantraCategory>;
}
...
function CategoryCard({ cat, sadhanaCount, isSelected, onClick }: { cat: TantraCategory; sadhanaCount: number; isSelected: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={cn(
...
/* ─── Sadhana Detail Card ─── */
function SadhanaCard({ sadhana, index, categoryMap }: { sadhana: Sadhana; index: number; categoryMap: Record<string, TantraCategory> }) {
  const [open, setOpen] = useState(false);
  const cat = categoryMap[sadhana.categoryId];
        ...
        >
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              ...
              <span className={cn('px-2 py-0.5 rounded border text-[0.55rem] font-mono tracking-wider', TIER_BADGE_STYLES[sadhana.minTier as Tier])}>
...
/* ─── MAIN PAGE ─── */
export default function SadhanaLibraryPage({ tantraCategories, sadhanaLibrary, sadhanaCount, siddhiCount, coursePhaseCount, categoryMap }: LibraryPageProps) {
  const reduced = useNativeReducedMotion();
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);

  const filteredSadhana = useMemo(() => {
    if (!selectedCategory) return sadhanaLibrary;
    return sadhanaLibrary.filter(s => s.categoryId === selectedCategory);
  }, [selectedCategory, sadhanaLibrary]);
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 mb-16">
          <div className="bg-foreground/5 border border-foreground/10 rounded-lg p-4">
            ...
            <p className="text-foreground text-sm font-medium">{sadhanaCount}</p>
          </div>
          <div className="bg-foreground/5 border border-foreground/10 rounded-lg p-4">
            <p className="text-caption text-xs mb-1">Categories</p>
            <p className="text-foreground text-sm font-medium">{tantraCategories.length}</p>
          </div>
          <div className="bg-foreground/5 border border-foreground/10 rounded-lg p-4">
            <p className="text-caption text-xs mb-1">Course Phases</p>
            <p className="text-foreground text-sm font-medium">{coursePhaseCount}</p>
          </div>
          <div className="bg-foreground/5 border border-foreground/10 rounded-lg p-4">
        ...
        >
          <div className="flex items-center gap-4 mb-4">
            <h2 className="font-display text-2xl md:text-3xl text-foreground font-light tracking-[0.04em] engraved-heading">
              {selectedCategory ? categoryMap[selectedCategory]?.name || 'All' : 'All'} Protocols
            </h2>
            <div className="flex-1 h-px bg-foreground/10" />
          ...
          >
            {filteredSadhana.map((sadhana, i) => (
              <SadhanaCard key={sadhana.slug} sadhana={sadhana} index={i} categoryMap={categoryMap} />
            ))}
          </motion.div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-16">
            <Link href="/archive" className="glass-chip p-5 group block hover:border-gold/20 transition-colors">
              ...
              <p className="text-foreground/40 text-xs">The complete siddhi database — {siddhiCount} folios with evidence grading</p>
            </Link>
