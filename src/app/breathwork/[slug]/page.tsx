...
/* ── Scale targets for each phase kind ── */
const SCALE_MAP: Record<PhaseKind, number> = {
  inhale: 1.45,
  retain: 1.45,
  exhale: 1.0,
};
...
const EASE_MAP: Record<PhaseKind, [number, number, number, number]> = {
  inhale: [0.25, 0.1, 0.25, 1],
  retain: [0.0, 0.0, 0.0, 1],
  exhale: [0.42, 0.0, 0.58, 1],
};
...
function BreathingCircle({
  ...
  /* Pre-compute animate values to avoid complex ternaries in JSX */
      ...
      <div className="flex flex-col items-center gap-6">
        <div className="relative w-[200px] h-[200px] md:w-[260px] md:h-[260px] flex items-center justify-center">
          <div
            ...
            style={{
              border: '2px solid rgba(197,164,75,0.4)',
              boxShadow: '0 0 30px rgba(197,164,75,0.15), inset 0 0 30px rgba(197,164,75,0.05)',
            }}
          />
          <div
            className="w-3/5 h-3/5 rounded-full"
            style={{
              background: 'radial-gradient(circle, rgba(197,164,75,0.12) 0%, rgba(197,164,75,0.03) 70%, transparent 100%)',
            }}
          />
        </div>
        <p className="font-display text-xl md:text-2xl text-gold tracking-[0.06em] font-light">
          {phaseName}
        </p>
      </div>
    );
    ...
    <div className="flex flex-col items-center gap-6">
      <div className="relative w-[200px] h-[200px] md:w-[260px] md:h-[260px] flex items-center justify-center">
        ...
        <motion.div
          ...
          animate={
            isActive && !isComplete
              ? { scale: [1, 1.06, 1], opacity: [glowOpacity * 0.5, glowOpacity, glowOpacity * 0.5] }
              : { opacity: 0.15 }
          }
          transition={{
            duration: phaseKind === 'retain' ? 1.6 : duration,
        ...
        <motion.div
          ...
          style={{
            border: '2px solid rgba(197,164,75,0.5)',
        ...
        >
          ...
          <motion.div
            ...
            animate={
              isActive && !isComplete
                ? { scale: targetScale, opacity: phaseKind === 'exhale' ? 0.3 : 0.7 }
                : { scale: 1, opacity: 0.3 }
            }
            transition={{
              duration,
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <motion.div
              ...
              transition={{
                duration: phaseKind === 'retain' ? 2 : duration,
                repeat: Infinity,
                ease: 'easeInOut',
              }}
            />
      ...
      <AnimatePresence mode="wait">
        <motion.p
          ...
          className="font-display text-xl md:text-2xl text-gold tracking-[0.06em] font-light"
        >
          {phaseName}
        </motion.p>
      </AnimatePresence>
      ...
      {isActive && !isComplete && (
        <motion.p
          ...
          className="font-mono text-4xl md:text-5xl text-foreground tabular-nums tracking-[0.08em]"
        >
          {Math.ceil(countdown)}
        </motion.p>
      )}
      ...
      {phases.map((phase, i) => {
        ...
        return (
          <div key={i} className="flex items-center gap-3">
            <span
              className={cn(
                'font-mono text-[0.7rem] tracking-[0.08em] w-28 md:w-40 text-right shrink-0 transition-colors duration-500',
                isCurrent ? 'text-gold' : isPast ? 'text-gold/40' : 'text-text-muted/40',
              )}
            >
              {phase.name}
            </span>
            <div className="flex-1 h-2 bg-surface/40 rounded-full overflow-hidden">
              <div
                className={cn(
                  ...
                  isCurrent && 'bg-gold neon-chip-glow',
                  isPast && 'bg-gold/30',
                  isFuture && 'bg-gold/8',
                )}
            ...
            <span
              className={cn(
                'font-mono text-[0.7rem] tracking-[0.08em] w-10 shrink-0 transition-colors duration-500',
                isCurrent ? 'text-gold' : 'text-text-muted/40',
              )}
            >
              {phase.duration}s
            </span>
...
/* ══════════════════════════════════════════════════════════════
   BREATH VISUALIZER DETAIL PAGE
   ══════════════════════════════════════════════════════════════ */
export default function BreathDetailPage({
  params,
}: {
      ...
      if (next <= 0) {
        ...
        setCurrentPhaseIndex((pi) => {
          ...
          if (nextPi >= totalPhasesPerCycle) {
            ...
            /* Reset to phase 0 of next cycle */
  ...
  /* ── Sync: stop running when complete ── */
  useEffect(() => {
    ...
    <div className="bg-deep-black min-h-screen">
      ...
      <header className="border-b border-gold/5">
        <div className="w-full max-w-4xl mx-auto px-6 lg:px-10 pt-32 pb-12">
          ...
          <motion.h1
            className="font-display text-4xl md:text-5xl text-foreground leading-[0.95] tracking-[0.06em] mb-4 font-light"
          ...
          >
            ...
            <span className="flex items-center gap-1.5 text-text-muted text-[0.75rem] font-mono tracking-[0.08em]">
              ...
              {pattern.phases.length}{pattern.phases.length === 1 ? ' phase' : ' phases'}
            </span>
          </motion.div>
        </div>
      </header>
      ...
      <section className="max-w-4xl mx-auto px-6 lg:px-10 pt-16 pb-20">
        <GatedContent
          minTier={pattern.minTier}
          label={`${pattern.name} — Visualizer`}
          teaser={`The animated breathing visualizer for ${pattern.name} is available to ${tierLabel} practitioners and above.`}
        >
          <motion.div
          ...
          >
            ...
            <div className="text-center mb-10">
              <p className="font-mono text-[0.75rem] tracking-[0.2em] uppercase text-text-muted">
              ...
              {isComplete && (
                ...
                >
                  <div className="inline-flex items-center gap-3 glass-chip px-6 py-4">
                    ...
                    <span className="font-display text-lg text-foreground font-light tracking-[0.04em]">
                      Session complete — {pattern.cycles} cycles finished.
                    </span>
                  </div>
                </motion.div>
              )}
            ...
            <div className="flex justify-center gap-3 mb-16">
              {!isRunning ? (
                <button
                  onClick={handleStart}
                  className="glass-chip px-6 py-2.5 text-sm font-mono tracking-[0.1em] uppercase text-gold hover:border-gold/30 transition-colors duration-300 flex items-center gap-2"
                  aria-label={isComplete ? 'Restart session' : 'Start session'}
                >
              ...
              <button
                onClick={handleReset}
                className="glass-chip px-6 py-2.5 text-sm font-mono tracking-[0.1em] uppercase text-text-muted hover:text-foreground hover:border-gold/20 transition-colors duration-300 flex items-center gap-2"
            ...
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <p className="section-label mb-6" style={{ letterSpacing: '0.4em' }}>
                Phase Map
              </p>
              <PhaseTimeline
          ...
          <div className="glass-chip p-6 md:p-8 overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-gold/10">
                  <th className="font-mono text-[0.7rem] tracking-[0.15em] uppercase text-text-muted pb-3 pr-4">
                    Phase
                  </th>
                  <th className="font-mono text-[0.7rem] tracking-[0.15em] uppercase text-text-muted pb-3 pr-4">
                    Duration
                  </th>
                  <th className="font-mono text-[0.7rem] tracking-[0.15em] uppercase text-text-muted pb-3">
                    Type
                  </th>
                </tr>
              </thead>
              <tbody>
                {pattern.phases.map((phase, i) => (
                  <tr key={i} className="border-b border-gold/5 last:border-0">
                    ...
                    <td className="py-3 pr-4 font-mono text-sm text-gold-dim">
                      {phase.duration}s
                    </td>
                    <td className="py-3 font-mono text-[0.75rem] tracking-[0.08em] uppercase text-text-muted">
                      {getPhaseKind(phase.name)}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <div className="mt-4 pt-4 border-t border-gold/10 flex flex-wrap items-center gap-x-6 gap-y-2">
              <span className="font-mono text-[0.75rem] text-text-muted">
                One cycle: <span className="text-gold-dim">{pattern.phases.reduce((a, p) => a + p.duration, 0)}s</span>
              </span>
              <span className="font-mono text-[0.75rem] text-text-muted">
                Total: <span className="text-gold-dim">{(pattern.phases.reduce((a, p) => a + p.duration, 0) * pattern.cycles).toFixed(0)}s</span>
              </span>
            </div>
          </div>
        </motion.div>
      ...
      <section className="relative py-24 md:py-36">
        ...
        >
          ...
          >
            ...
            explore the full prāṇāyāma laboratory.
          </p>
          <div className="flex flex-wrap justify-center gap-4">
            <Link href="/consultations" className="gold-cta">
              Consult the Archivist
            </Link>
            <Link href="/breathwork" className="ghost-cta">
              All Patterns
            </Link>
          </div>
        </ParallaxText>
      ...
      <footer className="relative pb-28 md:pb-20 mt-16">
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-10 text-center">
          ...
          <p className="font-mono text-[0.75rem] tracking-[0.2em] uppercase text-copper">
            AKASHIC ARCHIVE — PRĀṆĀYĀMA LABORATORY
          </p>
        </div>
      </footer>
    </div>
