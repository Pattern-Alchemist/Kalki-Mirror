      ...
      <div className="max-w-4xl mx-auto px-6 space-y-16 pb-32">
          ...
          <motion.section initial={reduced ? { opacity: 1 } : fadeInUp.hidden} whileInView={fadeInUp.visible} viewport={{ once: true }}>
            ...
            <Link href={`/archetypes#${archetype.id}`} className="glass-chip p-6 md:p-8 flex flex-col md:flex-row md:items-center gap-6 group hover:border-gold/30 transition-all duration-500 block">
              <div className="shrink-0 w-20 h-20 rounded-sm overflow-hidden border border-gold/10 relative">
                <CinematicImage src={archetype.image} alt={archetype.name} fill={false} width={80} height={80} filmGrain={false} vignette />
              </div>
            ...
            {pattern.signs.map((sign, i) => (
              <div key={i} className="glass-chip p-4 flex items-start gap-4">
                <div className="flex flex-col items-center gap-1 shrink-0 pt-1">
                  ...
                  <span className="font-mono text-[0.75rem] tracking-[0.15em] text-copper">{String(i + 1).padStart(2, '0')}</span>
                  <div className="w-px h-3" style={{ backgroundColor: 'var(--copper)' }} />
                </div>
        ...
        <ScrollParallax speed={-0.1} className="-mx-6 lg:-mx-10">
          <div className="relative h-[30vh] md:h-[35vh] overflow-hidden">
            <CinematicImage src="https://res.cloudinary.com/b9oo5abp/image/upload/f_auto,q_auto:good,w_1920,c_limit/kalki-mirror/tantra/archetypes/sri-yantra-mist" alt="Śrī Yantra" fill scrim="full" vignette filmGrain={false} />
          </div>
        </ScrollParallax>
        ...
        <motion.section initial={reduced ? { opacity: 1 } : fadeInUp.hidden} whileInView={fadeInUp.visible} viewport={{ once: true }}>
          ...
          <div className="glass-panel p-8 relative overflow-hidden">
            <div className="absolute inset-0 pointer-events-none opacity-[0.03]"
              style={{ backgroundImage: `repeating-linear-gradient(0deg, var(--copper) 0px, var(--copper) 1px, transparent 1px, transparent 40px), repeating-linear-gradient(90deg, var(--copper) 0px, var(--copper) 1px, transparent 1px, transparent 40px)` }}
            />
        ...
        {pattern.archetypeIntegration && patternTier && (
          <motion.section initial={reduced ? { opacity: 1 } : fadeInUp.hidden} whileInView={fadeInUp.visible} viewport={{ once: true }}>
            ...
            <GatedContent minTier='agni' label='Archetype Mapping' teaser={`Deep archetype analysis for ${pattern.name} is available to Agni practitioners and above.`}>
              <div className="glass-panel p-8"><p className="text-text-secondary leading-relaxed editorial-spacing">{pattern.archetypeIntegration}</p></div>
            </GatedContent>
          </motion.section>
        )}
        ...
        {pattern.advancedNotes && (
          <motion.section initial={reduced ? { opacity: 1 } : fadeInUp.hidden} whileInView={fadeInUp.visible} viewport={{ once: true }}>
            ...
            <GatedContent minTier='akash' label='Akash-Level Intelligence' teaser={`Advanced integration protocols for ${pattern.name} are available to Akash practitioners only.`}>
              <div className="glass-panel p-8 relative overflow-hidden">
                <div className="absolute inset-0 pointer-events-none opacity-[0.03]" style={{ backgroundImage: `repeating-linear-gradient(0deg, var(--copper) 0px, var(--copper) 1px, transparent 1px, transparent 40px), repeating-linear-gradient(90deg, var(--copper) 0px, var(--copper) 1px, transparent 1px, transparent 40px)` }} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {relatedSiddhis.map((s, i) => (
                <div key={s.slug} className="relative">
                  {i < relatedSiddhis.length - 1 && (<div className="hidden md:block absolute top-1/2 -right-2 w-4 h-px" style={{ backgroundColor: 'var(--copper)', opacity: 0.4 }} />)}
                  <Link href={`/archive/${s.slug}`} className="glass-chip p-5 hover:border-gold/30 transition-colors duration-500 group block">
