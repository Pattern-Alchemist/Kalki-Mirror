  ...
  return (
    ...
    >
      <div className="p-6 md:p-8">
        ...
        <p className="text-xs text-gold-dim mb-6 font-mono tracking-wider">{pattern.subtitle}</p>
        <div className="divider-subtle mb-5" />
        {/* Warning signs — use Cinnabar for critical patterns */}
        <ul className="space-y-2">
          {pattern.signs.slice(0, 3).map((s, i) => (
            <li key={i} className="text-xs text-text-secondary font-ui tracking-wide leading-relaxed">
              &middot; {s}
            </li>
          ))}
        </ul>
        ...
        <div className="mt-5 pt-3 border-t border-gold/5">
          <span className="font-mono text-[0.55rem] tracking-[0.2em] uppercase text-copper">
