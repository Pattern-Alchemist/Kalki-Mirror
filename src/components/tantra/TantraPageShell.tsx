// =============================================================
// KALKI — TANTRA EDUCATIONAL SCAFFOLD (server component)
// -------------------------------------------------------------
// Renders a TantraPage data entry with the house typography system:
...
// src/lib/data/tantra-pages.ts — this shell is typography only.
// =============================================================
...
import type { TantraPage } from '@/lib/data/tantra-pages';

const EMAIL_COURSE_HREF = '/email-course';

const CTA_MESSAGE =
  'Hello Kaustubh, I have a question about Tantra after reading on the site.';

interface TantraPageShellProps {
  page: TantraPage;
...
export function tantraPageMetadata(page: TantraPage): Metadata {
  return {
    title: page.title,
...
export function TantraPageShell({ page, crumbs, trackSlug, service }: TantraPageShellProps) {
      ...
      <div className="max-w-4xl mx-auto px-6 lg:px-10">
        ...
        <section className="mb-16 border border-gold-dim/30 rounded-sm px-6 py-10 md:px-10 bg-foreground/[0.03]">
          ...
          <p className="text-editorial text-foreground/80 leading-relaxed mb-8 max-w-2xl">
            A consultation with Kaustubh locates this material in your own pattern and
            prescribes the practice that addresses it — the structured route is the Aghorī
            Tantra course, and the Ten Doors email course walks the whole framework in ten
            days, free.
          </p>
          <div className="flex flex-wrap gap-3">
            <WhatsAppCTA variant="inline" label="Ask Kaustubh Directly" message={CTA_MESSAGE} topic={page.topic} />
            <Link href="/aghori-tantra" className="ghost-cta text-sm">
              The Aghorī Tantra Course
            </Link>
            <Link href={EMAIL_COURSE_HREF} className="ghost-cta text-sm">
              Start the Ten Doors
            </Link>
