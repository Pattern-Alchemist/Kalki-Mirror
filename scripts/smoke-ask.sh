...
# 4. rate limit: 6 rapid hits, at least one 429 (window 5/min per IP)
RL=0
CODES=""
for i in 1 2 3 4 5 6; do
  C=$(curl -s -o /dev/null -w '%{http_code}' -m 60 -X POST "${BASE_URL}/api/ai/ask" -H "Content-Type: application/json" -d '{"query":"rate limit probe number '"$i"'"}')
  CODES="$CODES $C"
  [ "$C" = "429" ] && RL=1
done
check "rate limit engages within 6 rapid calls" "$RL" "codes:$CODES"
